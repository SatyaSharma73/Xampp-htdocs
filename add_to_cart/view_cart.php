...
<form method="post" action="payment.php">
	...
	<table>
		<tr><td>ID</td><td>Name</td><td>Price</td><td>Quantity</td><td>Sub Total</td><td>Actions</td></tr>
		<?php

		$total=0;
		$index=1;
		$_SESSION['cart']=session_id();
		$query1 = "SELECT distinct p_id FROM order_session WHERE session='".$_SESSION['cart']."'";
		$result = mysqli_query($con,$query1);
		while ($fetch=mysqli_fetch_object($result)) {
		$query2="select id,name,price from items where id='".$fetch->p_id."'";
	...
	<td>
	<?php
	$query3 = "SELECT distinct id FROM order_session WHERE p_id='".$fetch->p_id."' and session='".$_SESSION['cart']."'";
	...
	<td><a href="cart.php?increase=<?php echo $fetch1->id;?>">Increase</a></td>
	<td><a href="cart.php?decrease=<?php echo $fetch1->id;?>">Decrease</a></td>
	<td><a href="cart.php?delete=<?php echo $fetch1->id;?>">Delete</a></td>
		</tr>
		<?php
		$index++;
		}?>
	...
	<table>
		...
		<tr><td><input type="hidden" name="surl" value="http://localhost/add_to_cart/success.php"></td></tr>
		<tr><td><input type="hidden" name="furl" value="http://localhost/add_to_cart/failure.php"></td></tr>
